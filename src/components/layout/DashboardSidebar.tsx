...
import {
  LayoutDashboard, Store, Megaphone, FileText, MessageSquare,
  Ticket, Package, Users, ShieldCheck, LogOut, ShoppingBag,
  ChevronLeft, ChevronRight, Bell, Sprout, ShieldPlus,
} from 'lucide-react';
...
const ADMIN_LINKS: NavLink[] = [
  ...
  { href: '/admin/coberturas', label: 'Coberturas', icon: ShieldPlus },
  { href: '/admin/promociones', label: 'Promociones', icon: Bell },
  { href: '/admin/tickets', label: 'Tickets', icon: Ticket },
  { href: '/admin/seed', label: 'Datos demo', icon: Sprout },
];

const COMERCIO_LINKS: NavLink[] = [
  ...
  { href: '/comercio/seguros', label: 'Coberturas', icon: ShieldCheck },
  { href: '/comercio/productos', label: 'Catálogo', icon: Package, soon: true },
];
...
export default function DashboardSidebar() {
  ...
  return (
    <aside className={`flex flex-col bg-slate-900 text-white transition-all duration-300 ${collapsed ? 'w-16' : 'w-60'} min-h-screen shrink-0`}>
      {/* Header */}
      ...
      {!collapsed && usuario && (
        <div className="px-4 py-3 border-b border-slate-700">
          ...
          <p className="text-sm font-medium text-white truncate">{usuario.displayName ?? usuario.email}</p>
      ...
      {/* Footer */}
      <div className="px-2 py-4 border-t border-slate-700 space-y-0.5">
        <Link
          href="/"
